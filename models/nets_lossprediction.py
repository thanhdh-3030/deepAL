...
def LossPredLoss(input, target, margin=1.0, reduction='mean'):
    # assert len(input) % 2 == 0, 'the batch size is not even.' edited that code
...
class Net_LPL:
    ...
    def train(self, data, weight = 1.0, margin = 1.0 , lpl_epoch = 120):
        ...
        optimizer = optim.SGD(self.clf.parameters(), lr=LR, 
                                    momentum=MOMENTUM, weight_decay=WDECAY)
        optimizer_lpl = optim.SGD(self.clf_lpl.parameters(), lr=LR, 
                                    momentum=MOMENTUM, weight_decay=WDECAY)
        sched_backbone = lr_scheduler.MultiStepLR(optimizer, milestones=MILESTONES)
        sched_module   = lr_scheduler.MultiStepLR(optimizer_lpl, milestones=MILESTONES)
        ...
        for epoch in tqdm(range(1, n_epoch+1), ncols=100):
            ...
            for batch_idx, (x,x1, y, idxs) in enumerate(loader):
                ...
                out, features = self.clf.forward_ll(x)
                cross_ent = nn.CrossEntropyLoss(reduction='none')
                target_loss = cross_ent(out,y) # main loss
                if epoch >= epoch_loss:
                        features[0] = features[0].detach()
                        features[1] = features[1].detach()
                        features[2] = features[2].detach()
                        features[3] = features[3].detach()
                pred_loss = self.clf_lpl(features) # loss prediction loss
...
class CIFAR10_Net_LPL(nn.Module):
    def __init__(self, dim = 28 * 28, pretrained=False, num_classes = 10):
        ...
        self.feature0 = nn.Sequential(*list(resnet18.children())[1:3])
        self.feature1 = nn.Sequential(*list(resnet18.children())[4])
        self.feature2 = nn.Sequential(*list(resnet18.children())[5])
        self.feature3 = nn.Sequential(*list(resnet18.children())[6]) 
        self.feature4 = nn.Sequential(*list(resnet18.children())[7])
